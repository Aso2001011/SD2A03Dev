<header>
    <h1><a href="index.php">T　I　T　L　E</a></h1>
    <nav class="menu">
        <ul class="nav_menu1">
            <li class="nav_menu1_item"><a href="newreg.php" class="">新規会員登録</a></li>
            <li class="nav_menu1_item"><a href="" class="">ご利用ガイド</a></li>
            <li class="nav_menu1_item"><a href="" class="">お問い合わせ</a></li>
            <li class="nav_menu1_item"><a href="" class="">サポート</a></li>
        </ul>
        <ul class="nav_menu2">
            <li class="nav_menu2_item login"><a href="login.php" class=""><i class="fas fa-sign-in-alt"></i> ログイン</a></li>
            <li class="nav_menu2_item"><a href="info.php" class=""><i class="fas fa-user-circle"></i> 会員情報</a></li>
            <li class="nav_menu2_item"><a href="fav.php" class=""><i class="fas fa-heart"></i> お気に入り</a></li>
            <li class="nav_menu2_item"><a href="cart.php" class=""><i class="fas fa-shopping-cart"></i> カート</a></li>
            <li class="nav_menu2_item glass">
                <form action="search.php" method="post">
                    <input type="text" name="search" class="search" placeholder="何をお探しですか？">
                    <button type="submit"><i class="fas fa-search"></i></button><span>検索</span>
                </form>
            </li>
        </ul>
        <ul class="nav_menu3">
            <li class="nav_menu3_item"><a href="search.php" class="" onclick="form_1()">デスクトップPC</a></li>
            <li class="nav_menu3_item"><a href="search.php" class="" onclick="form_2()">ノートPC</a></li>
            <li class="nav_menu3_item"><a href="search.php" class="" onclick="form_3()">ゲーミングPC</a></li>
            <li class="nav_menu3_item"><a href="search.php" class="" onclick="form_4()">クリエイター向けPC</a></li>
        </ul>
    </nav>
</header>
<div class="cont">